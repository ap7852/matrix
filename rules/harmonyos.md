...
- `@Provide`: 父组件提供状态
- `@Consume`: 子组件消费状态
- **重要**: `@Provide` 已包含状态管理，不能与 `@State` 组合使用
...
// 父组件 - 错误 (双重装饰器)
@Provide('isAuthenticated') @State isAuthenticated: boolean = false  // 编译错误
...
### @Reusable - 组件复用优化
- 列表渲染时组件实例复用
- 需要 `aboutToReuse()` 重置状态

```typescript
@Reusable
@Component
struct RoomListItem {
  @Prop room: RoomSummary

  aboutToReuse(params: RoomSummary): void {
    this.room = params
  }
}
```
...
- **大列表**: 必须使用 `LazyForEach` + `IDataSource`
- **小列表 (< 100项)**: 可用 `ForEach`
- 禁止在超过 100 项的列表使用 `ForEach`
...
const dataDir = context.filesDir
